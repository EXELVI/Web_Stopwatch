            ...
            if (client.available())
            {
                ...
                if (c == '\n')
                {
                    if (currentLine.length() == 0)
                    {
                        ...
                        client.println("            const seconds = Math.floor(elapsedTime / 1000) % 60, minutes = Math.floor(elapsedTime / (1000 * 60)) % 60, hours = Math.floor(elapsedTime / (1000 * 60 * 60)) % 24, ms = elapsedTime % 1000; display = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')},${String(ms).padStart(3, '0')}`;");
                        ...
                        client.println("            updateTimerDisplay(data.elapsedTime);");
                        client.println("            if (data.running) {");
                        client.println("                intervalId = setInterval(async () => {");
                        client.println("                    const res = await fetch('/data'), data = await res.json();");
                        client.println("                    updateTimerDisplay(data.elapsedTime);");
                        client.println("                }, 1000);");
                        client.println("            } else clearInterval(intervalId);");
                        client.println("        }");
                        client.println("        document.getElementById('startButton').addEventListener('click', async () => { await fetch('/start'); fetchData(); });");
                        client.println("        document.getElementById('stopButton').addEventListener('click', async () => { await fetch('/stop'); fetchData(); });");
                        client.println("        document.getElementById('resetButton').addEventListener('click', async () => { await fetch('/reset'); fetchData(); });");
                        client.println("");
